...
from app.core.db.models import User, Message
from app.core.agents.chemagent import get_chemagent_instance, ChemAgent
...
class ChemAgentRequest(BaseModel):
    ...
    conversation_id: Optional[str] = Field(None, description="对话ID，如果不提供将自动生成")
    model: str = Field(default=os.getenv("DEFAULT_MODEL", "deepseek-chat"), description="使用的LLM模型")
    tools_model: str = Field(default=os.getenv("DEFAULT_TOOLS_MODEL", "deepseek-chat"), description="工具选择使用的模型")
    temperature: float = Field(default=float(os.getenv("DEFAULT_TEMPERATURE", "0.1")), description="模型温度参数")
    max_iterations: int = Field(default=int(os.getenv("DEFAULT_MAX_ITERATIONS", "40")), description="最大迭代次数")
    streaming: bool = Field(default=False, description="是否使用流式输出")
    local_rxn: bool = Field(default=False, description="是否使用本地反应工具")
    api_keys: Dict[str, str] = Field(default={}, description="API密钥配置")
            ...
            try:
                ...
                async for chunk in chemagent.run_stream(enhanced_prompt):
                    ...
                    try:
                        ...
                        if chunk_data.get("type") == "final":
                            ...
                            thinking_steps.append(chunk_data) # 存储思考步骤
                ...
                if assistant_response:
                    assistant_message = add_message(
                        db, conv_id, "assistant", assistant_response, 
                        request.model, steps=thinking_steps
                    )
                else:
                    ...
                    if hasattr(chemagent, 'streaming_callback') and chemagent.streaming_callback.tokens:
                        ...
                        if fallback_response.strip():
                            assistant_message = add_message(
                                db, conv_id, "assistant", fallback_response, 
                                request.model, steps=thinking_steps
                            )
...
def get_conversations(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    for conv in conversations:
        ...
        result.append({
            "id": conv.conversation_id,  # 使用conversation_id作为前端的id
            "title": conv.title,
            "model_used": conv.model_used,  # 修复：使用model_used而不是model
            "created_at": conv.created_at.isoformat(),  # 确保日期格式正确
            "updated_at": conv.updated_at.isoformat() if conv.updated_at else conv.created_at.isoformat(),
            "message_count": message_count  # 实际计算消息数量
        })
...
def get_conversation(
    ...
    messages_in_db = []
    for msg in messages:
        steps = getattr(msg, 'steps', [])
        messages_in_db.append(
            MessageInDB(
                id=msg.id,
                conversation_id=conversation.conversation_id,  # 这是字符串UUID
                role=msg.role,
                content=msg.content,
...
def update_conversation_title_endpoint(
    ...
    return {"message": "Title updated successfully", "conversation_id": updated_conv.conversation_id, "new_title": updated_conv.title}
...
def delete_conversation_endpoint(
    ...
    # 删除与对话相关的向量
    vector_store.delete_conversation_vectors(db, conversation_id)
...
def get_models():
    return {
        "models": [
            ...
            {"id": "gpt-3.5-turbo-0613", "name": "GPT-3.5-Turbo"}
        ]
    }
